...
import { c as create_ssr_component, a as createEventDispatcher, b as add_attribute, d as add_styles, f as spread, g as escape_object, e as escape, o as onDestroy, h as each, v as validate_component } from "../../../../chunks/ssr.js";
...
function createGlowFilter(effect) {
  const id = `glow-${Math.random().toString(36).slice(2)}`;
  Math.floor(effect.intensity * 255).toString(16).padStart(2, "0");
  Math.floor(effect.intensity * 1.5 * 255).toString(16).padStart(2, "0");
  const element = `
        <filter id="${id}" x="-100%" y="-100%" width="300%" height="300%">
            <feGaussianBlur in="SourceAlpha" stdDeviation="${effect.radius - 20}" result="blur"/>
            <feFlood flood-color="${effect.color}" flood-opacity="${0}" result="color"/>
...
function createGlowGradient(effect) {
...
const css$l = {
  code: ".navigation-node.svelte-13p5e86{cursor:pointer}.icon-container.svelte-13p5e86{overflow:visible;transition:transform 0.2s ease-out}.icon-wrapper.svelte-13p5e86{width:100%;height:100%;display:flex;align-items:center;justify-content:center}.navigation-node .material-symbols-outlined{font-size:24px;transition:font-size 0.2s ease-out, color 0.3s ease}.navigation-node:hover .material-symbols-outlined{font-size:32px}.label.svelte-13p5e86{font-family:'Orbitron', sans-serif;font-size:14px;text-anchor:middle;dominant-baseline:middle;transition:fill 0.3s ease}.connection-line.svelte-13p5e86{pointer-events:none;vector-effect:non-scaling-stroke}.glow.svelte-13p5e86{pointer-events:none}.navigation-node *{vector-effect:non-scaling-stroke}",
  map: '{"version":3,"file":"NavigationNode.svelte","sources":["NavigationNode.svelte"],"sourcesContent":["<script lang=\\"ts\\">import { createEventDispatcher } from \\"svelte\\";\\nimport { getNavigationColor } from \\"./navigationColors\\";\\nimport { createGlowFilter, createGlowGradient } from \\"$lib/utils/svgAnimations\\";\\nconst dispatch = createEventDispatcher();\\nexport let option;\\nexport let transform;\\nexport let isHovered = false;\\nconst color = getNavigationColor(option.id);\\nconst glowEffect = {\\n  color,\\n  radius: 10,\\n  intensity: 0.1,\\n  fade: true\\n};\\nconst { id: filterId, element: filterElement } = createGlowFilter(glowEffect);\\nconst { id: gradientId, element: gradientElement } = createGlowGradient(glowEffect);\\nlet transformValues = [];\\nlet translateX = 0;\\nlet translateY = 0;\\n$: {\\n  const matches = transform.match(/translate\\\\(([-\\\\d.e+-]+),\\\\s*([-\\\\d.e+-]+)\\\\)/);\\n  if (matches) {\\n    transformValues = [parseFloat(matches[1]), parseFloat(matches[2])];\\n    [translateX, translateY] = transformValues;\\n  } else {\\n    transformValues = [0, 0];\\n    [translateX, translateY] = transformValues;\\n  }\\n}\\n<\/script>\\n\\n<!-- svelte-ignore a11y-click-events-have-key-events -->\\n<!-- svelte-ignore a11y-no-static-element-interactions -->\\n<g \\n    class=\\"navigation-node\\"\\n    {transform}\\n    on:mouseenter\\n    on:mouseleave\\n    on:click\\n>\\n    <defs>\\n        {@html filterElement}\\n        {@html gradientElement}\\n    </defs>\\n\\n    {#if isHovered}\\n        <!-- Connection line to center -->\\n        <line \\n            class=\\"connection-line\\"\\n            x1=\\"0\\"\\n            y1=\\"0\\"\\n            x2={-translateX}\\n            y2={-translateY}\\n            stroke={`${color}50`}\\n            stroke-width=\\"1\\"\\n        />\\n\\n        <!-- Glow effect -->\\n        <circle\\n            class=\\"glow\\"\\n            r=\\"25\\"\\n            fill={`url(#${gradientId})`}\\n            filter={`url(#${filterId})`}\\n        />\\n    {/if}\\n\\n    <!-- Icon Container -->\\n    <foreignObject \\n        x=\\"-16\\" \\n        y=\\"-16\\" \\n        width=\\"32\\" \\n        height=\\"32\\" \\n        class=\\"icon-container\\"\\n        style:filter={isHovered ? `url(#${filterId})` : \'none\'}\\n    >\\n        <!-- svelte-ignore a11y-no-static-element-interactions -->\\n        <div \\n            class=\\"icon-wrapper\\"\\n            {...{\\"xmlns\\": \\"http://www.w3.org/1999/xhtml\\"}}\\n        >\\n            <span \\n                class=\\"material-symbols-outlined\\"\\n                style:color={isHovered ? color : \'white\'}\\n            >\\n                {option.icon}\\n            </span>\\n        </div>\\n    </foreignObject>\\n\\n    <!-- Label -->\\n    {#if isHovered}\\n        <text\\n            class=\\"label\\"\\n            dy=\\"30\\"\\n            style:fill={color}\\n        >\\n            {option.label}\\n        </text>\\n    {/if}\\n</g>\\n\\n<style>\\n    .navigation-node {\\n        cursor: pointer;\\n    }\\n\\n    .icon-container {\\n        overflow: visible;\\n        transition: transform 0.2s ease-out;\\n    }\\n\\n    .icon-wrapper {\\n        width: 100%;\\n        height: 100%;\\n        display: flex;\\n        align-items: center;\\n        justify-content: center;\\n    }\\n\\n    :global(.navigation-node .material-symbols-outlined) {\\n        font-size: 24px;\\n        transition: font-size 0.2s ease-out, color 0.3s ease;\\n    }\\n\\n    :global(.navigation-node:hover .material-symbols-outlined) {\\n        font-size: 32px;\\n    }\\n\\n    .label {\\n        font-family: \'Orbitron\', sans-serif;\\n        font-size: 14px;\\n        text-anchor: middle;\\n        dominant-baseline: middle;\\n        transition: fill 0.3s ease;\\n    }\\n\\n    .connection-line {\\n        pointer-events: none;\\n        vector-effect: non-scaling-stroke;\\n    }\\n\\n    .glow {\\n        pointer-events: none;\\n    }\\n\\n    :global(.navigation-node *) {\\n        vector-effect: non-scaling-stroke;\\n    }\\n</style>"],"names":[],"mappings":"AAsGI,+BAAiB,CACb,MAAM,CAAE,OACZ,CAEA,8BAAgB,CACZ,QAAQ,CAAE,OAAO,CACjB,UAAU,CAAE,SAAS,CAAC,IAAI,CAAC,QAC/B,CAEA,4BAAc,CACV,KAAK,CAAE,IAAI,CACX,MAAM,CAAE,IAAI,CACZ,OAAO,CAAE,IAAI,CACb,WAAW,CAAE,MAAM,CACnB,eAAe,CAAE,MACrB,CAEQ,2CAA6C,CACjD,SAAS,CAAE,IAAI,CACf,UAAU,CAAE,SAAS,CAAC,IAAI,CAAC,QAAQ,CAAC,CAAC,KAAK,CAAC,IAAI,CAAC,IACpD,CAEQ,iDAAmD,CACvD,SAAS,CAAE,IACf,CAEA,qBAAO,CACH,WAAW,CAAE,UAAU,CAAC,CAAC,UAAU,CACnC,SAAS,CAAE,IAAI,CACf,WAAW,CAAE,MAAM,CACnB,iBAAiB,CAAE,MAAM,CACzB,UAAU,CAAE,IAAI,CAAC,IAAI,CAAC,IAC1B,CAEA,+BAAiB,CACb,cAAc,CAAE,IAAI,CACpB,aAAa,CAAE,kBACnB,CAEA,oBAAM,CACF,cAAc,CAAE,IACpB,CAEQ,kBAAoB,CACxB,aAAa,CAAE,kBACnB"}'
};
const NavigationNode = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `  <g class="navigation-node svelte-13p5e86"${add_attribute("transform", transform, 0)}><defs><!-- HTML_TAG_START -->${filterElement}<!-- HTML_TAG_END --><!-- HTML_TAG_START -->${gradientElement}<!-- HTML_TAG_END --></defs>${isHovered ? ` <line class="connection-line svelte-13p5e86" x1="0" y1="0"${add_attribute("x2", -translateX, 0)}${add_attribute("y2", -translateY, 0)}${add_attribute("stroke", `${color}50`, 0)} stroke-width="1"></line>  <circle class="glow svelte-13p5e86" r="25"${add_attribute("fill", `url(#${gradientId})`, 0)}${add_attribute("filter", `url(#${filterId})`, 0)}></circle>` : ``}<foreignObject x="-16" y="-16" width="32" height="32" class="icon-container svelte-13p5e86"${add_styles({
    ...
    { classes: "svelte-13p5e86" }
  )}><span class="material-symbols-outlined"${add_styles({ "color": isHovered ? color : "white" })}>${escape(option.icon)}</span></div></foreignObject>${isHovered ? `<text class="label svelte-13p5e86" dy="30"${add_styles({ "fill": color })}>${escape(option.label)}</text>` : ``}</g>`;
});
function isDashboardNode(node) {
  return node.type === "dashboard";
}
function isEditProfileNode(node) {
  return node.type === "edit-profile";
}
function isCreateNodeNode(node) {
...
const GraphLayout_1 = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.resetView === void 0 && $$bindings.resetView && resetView !== void 0) $$bindings.resetView(resetView);
  ...
  {
    {
      console.log("Edge rendering state:", {
        ...
        nodePositions: Object.fromEntries(Array.from(nodePositions.entries()).map(([id, pos]) => [id, { x: pos.x, y: pos.y }]))
      });
    }
  }
  return `  <svg class="graph svelte-190ehr9"${add_attribute("width", width, 0)}${add_attribute("height", height, 0)} viewBox="${"0 0 " + escape(width, true) + " " + escape(height, true)}"${add_attribute("this", svg, 0)}><defs><radialGradient id="node-gradient" cx="50%" cy="50%" r="50%"><stop offset="0%" stop-color="rgba(255,255,255,0.1)"></stop><stop offset="100%" stop-color="rgba(255,255,255,0)"></stop></radialGradient></defs><g${add_attribute("this", container, 0)}>${each(links, (link) => {
    let sourceId = typeof link.source === "string" ? link.source : "id" in link.source ? link.source.id : null, targetId = typeof link.target === "string" ? link.target : "id" in link.target ? link.target.id : null, sourcePos = sourceId ? nodePositions.get(sourceId) : null, targetPos = targetId ? nodePositions.get(targetId) : null;
    return `    ${sourcePos && targetPos ? `${slots.edge ? slots.edge({
      link,
      source: sourcePos,
      target: targetPos
    }) : ``}` : ``}`;
    ...
    return ` ${position ? `${isNavigationNode(node) ? `${validate_component(NavigationNode, "NavigationNode").$$render(
      $$result,
      {
        option: node.data,
        transform: position.svgTransform,
      ...
      {},
      {}
    )}` : `<g class="node"${add_attribute("transform", position.svgTransform, 0)}${add_attribute("data-node-id", node.id, 0)}>${slots.node ? slots.node({ node, position }) : ``}</g>`}` : ``}`;
  })}</g></svg>`;
});
...
const css$j = {
  code: ".edge-line.svelte-kov06k{vector-effect:non-scaling-stroke}.edge.svelte-kov06k{pointer-events:none}",
  map: '{"version":3,"file":"Edge.svelte","sources":["Edge.svelte"],"sourcesContent":["<!-- src/lib/components/graph/edges/Edge.svelte -->\\n<script lang=\\"ts\\">import { EDGE_CONSTANTS } from \\"./EdgeConstants\\";\\nexport let source;\\nexport let target;\\nexport let link;\\nconst gradientId = `edge-gradient-${Math.random().toString(36).slice(2)}`;\\n$: color = link.type === \\"live\\" ? EDGE_CONSTANTS.COLORS.DEFINITION.LIVE : EDGE_CONSTANTS.COLORS.DEFINITION.ALTERNATIVE;\\n<\/script>\\n\\n<g class=\\"edge\\">\\n    <defs>\\n        <linearGradient \\n            id={gradientId} \\n            gradientUnits=\\"userSpaceOnUse\\"\\n            x1={source.x}\\n            y1={source.y}\\n            x2={target.x}\\n            y2={target.y}\\n        >\\n            <stop \\n                offset=\\"0%\\" \\n                stop-color={EDGE_CONSTANTS.COLORS.WORD} \\n                stop-opacity=\\"0.8\\"\\n            />\\n            <stop \\n                offset=\\"100%\\" \\n                stop-color={color} \\n                stop-opacity=\\"0.8\\"\\n            />\\n        </linearGradient>\\n    </defs>\\n\\n    <line\\n        x1={source.x}\\n        y1={source.y}\\n        x2={target.x}\\n        y2={target.y}\\n        stroke={`url(#${gradientId})`}\\n        stroke-width=\\"2\\"\\n        class=\\"edge-line\\"\\n    />\\n</g>\\n\\n<style>\\n    .edge-line {\\n        vector-effect: non-scaling-stroke;\\n    }\\n\\n    .edge {\\n        pointer-events: none;\\n    }\\n</style>"],"names":[],"mappings":"AA4CI,wBAAW,CACP,aAAa,CAAE,kBACnB,CAEA,mBAAM,CACF,cAAc,CAAE,IACpB"}'
};
const Edge = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let color;
  let { source } = $$props;
  let { target } = $$props;
  let { link } = $$props;
  ...
  color = link.type === "live" ? EDGE_CONSTANTS.COLORS.DEFINITION.LIVE : EDGE_CONSTANTS.COLORS.DEFINITION.ALTERNATIVE;
  return `  <g class="edge svelte-kov06k"><defs><linearGradient${add_attribute("id", gradientId, 0)} gradientUnits="userSpaceOnUse"${add_attribute("x1", source.x, 0)}${add_attribute("y1", source.y, 0)}${add_attribute("x2", target.x, 0)}${add_attribute("y2", target.y, 0)}><stop offset="0%"${add_attribute("stop-color", EDGE_CONSTANTS.COLORS.WORD, 0)} stop-opacity="0.8"></stop><stop offset="100%"${add_attribute("stop-color", color, 0)} stop-opacity="0.8"></stop></linearGradient></defs><line${add_attribute("x1", source.x, 0)}${add_attribute("y1", source.y, 0)}${add_attribute("x2", target.x, 0)}${add_attribute("y2", target.y, 0)}${add_attribute("stroke", `url(#${gradientId})`, 0)} stroke-width="2" class="edge-line svelte-kov06k"></line></g>`;
});
const NODE_STYLE = {
  RADIUS: 3,
  GLOW_RADIUS: 5,
...
const css$i = {
  code: ".graph-container.svelte-1vitirz{width:100%;height:100%;position:relative;overflow:hidden;background-color:black}.background-svg.svelte-1vitirz{position:absolute;top:0;left:0;width:100%;height:100%;z-index:1;pointer-events:none;overflow:visible}.reset-button.svelte-1vitirz{position:absolute;bottom:20px;right:20px;width:40px;height:40px;border-radius:50%;background:rgba(0, 0, 0, 0.7);border:1px solid rgba(255, 255, 255, 0.2);color:rgba(255, 255, 255, 0.8);font-size:20px;cursor:pointer;display:flex;align-items:center;justify-content:center;z-index:10;transition:all 0.3s ease-out}.reset-button.svelte-1vitirz:hover{background:rgba(0, 0, 0, 0.8);border-color:rgba(255, 255, 255, 0.4);color:rgba(255, 255, 255, 1)}.reset-button.svelte-1vitirz:active{transform:scale(0.95)}.graph-container svg{width:100%;height:100%}.graph-container .node{transition:transform 0.3s ease-out}.graph-container .edge{pointer-events:none}",
  map: '{"version":3,"file":"Graph.svelte","sources":["Graph.svelte"],"sourcesContent":["<!-- src/lib/components/graph/Graph.svelte -->\\n<script lang=\\"ts\\">import { onMount, onDestroy, createEventDispatcher } from \\"svelte\\";\\nimport { browser } from \\"$app/environment\\";\\nimport GraphLayout from \\"../graph/layouts/GraphLayout.svelte\\";\\nimport Edge from \\"./edges/Edge.svelte\\";\\nimport { SvgBackground } from \\"./backgrounds/SvgBackground\\";\\nimport { DEFAULT_BACKGROUND_CONFIG } from \\"./backgrounds/backgroundConfig\\";\\nexport let nodes = [];\\nexport let links = [];\\nexport let width = window.innerWidth;\\nexport let height = window.innerHeight;\\nexport let backgroundConfig = {};\\nexport let isPreviewMode = false;\\nconst mergedConfig = { ...DEFAULT_BACKGROUND_CONFIG, ...backgroundConfig };\\nconst dispatch = createEventDispatcher();\\nlet container;\\nlet backgroundSvg;\\nlet backgroundGroup;\\nlet background = null;\\nlet graphLayout;\\nfunction updateDimensions() {\\n  if (!container) return;\\n  const rect = container.getBoundingClientRect();\\n  width = rect.width;\\n  height = rect.height;\\n  if (background) {\\n    background.resize(width, height);\\n  }\\n}\\nfunction initializeBackground() {\\n  if (!browser || !backgroundGroup) return;\\n  background = new SvgBackground(\\n    backgroundGroup,\\n    width,\\n    height\\n  );\\n  if (background) {\\n    background.start();\\n  }\\n}\\nfunction handleReset() {\\n  if (graphLayout) {\\n    graphLayout.resetView();\\n  }\\n}\\nonMount(() => {\\n  updateDimensions();\\n  window.addEventListener(\\"resize\\", updateDimensions);\\n  if (width && height) {\\n    initializeBackground();\\n  }\\n});\\nonDestroy(() => {\\n  window.removeEventListener(\\"resize\\", updateDimensions);\\n  if (background) {\\n    background.destroy();\\n  }\\n});\\n$: if (isPreviewMode !== void 0) {\\n  console.log(\\"Graph.svelte - Preview mode changed:\\", isPreviewMode);\\n  if (graphLayout) {\\n    setTimeout(() => {\\n      graphLayout.resetView();\\n    }, 0);\\n  }\\n}\\n$: viewBox = width ? `${width * mergedConfig.viewport.origin.x} ${height * mergedConfig.viewport.origin.y} ${width * mergedConfig.viewport.scale} ${height * mergedConfig.viewport.scale}` : \\"0 0 100 100\\";\\n<\/script>\\n \\n<div \\n    class=\\"graph-container\\"\\n    bind:this={container}\\n>\\n    <button \\n        class=\\"reset-button\\"\\n        on:click={handleReset}\\n        aria-label=\\"Reset view\\"\\n    >\\n        ⟲\\n    </button>\\n \\n    <svg \\n        bind:this={backgroundSvg}\\n        class=\\"background-svg\\"\\n        {width}\\n        {height}\\n        {viewBox}\\n        preserveAspectRatio={mergedConfig.viewport.preserveAspectRatio}\\n    >\\n        <g \\n            bind:this={backgroundGroup}\\n            class=\\"background-group\\"\\n        />\\n    </svg>\\n \\n    <GraphLayout \\n        bind:this={graphLayout}\\n        {nodes}\\n        {links}\\n        {width} \\n        {height}\\n        {isPreviewMode}\\n        on:modeChange\\n    >\\n        <svelte:fragment slot=\\"edge\\" let:link let:source let:target>\\n            <Edge {link} {source} {target} />\\n        </svelte:fragment>\\n\\n        <svelte:fragment slot=\\"node\\" let:node let:position>\\n            <slot \\n                name=\\"node\\" \\n                {node}\\n                {position}\\n            />\\n        </svelte:fragment>\\n    </GraphLayout>\\n</div>\\n \\n<style>\\n    .graph-container {\\n        width: 100%;\\n        height: 100%;\\n        position: relative;\\n        overflow: hidden;\\n        background-color: black;\\n    }\\n \\n    .background-svg {\\n        position: absolute;\\n        top: 0;\\n        left: 0;\\n        width: 100%;\\n        height: 100%;\\n        z-index: 1;\\n        pointer-events: none;\\n        overflow: visible;\\n    }\\n \\n    .reset-button {\\n        position: absolute;\\n        bottom: 20px;\\n        right: 20px;\\n        width: 40px;\\n        height: 40px;\\n        border-radius: 50%;\\n        background: rgba(0, 0, 0, 0.7);\\n        border: 1px solid rgba(255, 255, 255, 0.2);\\n        color: rgba(255, 255, 255, 0.8);\\n        font-size: 20px;\\n        cursor: pointer;\\n        display: flex;\\n        align-items: center;\\n        justify-content: center;\\n        z-index: 10;\\n        transition: all 0.3s ease-out;\\n    }\\n \\n    .reset-button:hover {\\n        background: rgba(0, 0, 0, 0.8);\\n        border-color: rgba(255, 255, 255, 0.4);\\n        color: rgba(255, 255, 255, 1);\\n    }\\n \\n    .reset-button:active {\\n        transform: scale(0.95);\\n    }\\n \\n    :global(.graph-container svg) {\\n        width: 100%;\\n        height: 100%;\\n    }\\n \\n    :global(.graph-container .node) {\\n        transition: transform 0.3s ease-out;\\n    }\\n \\n    :global(.graph-container .edge) {\\n        pointer-events: none;\\n    }\\n</style>"],"names":[],"mappings":"AAuHI,+BAAiB,CACb,KAAK,CAAE,IAAI,CACX,MAAM,CAAE,IAAI,CACZ,QAAQ,CAAE,QAAQ,CAClB,QAAQ,CAAE,MAAM,CAChB,gBAAgB,CAAE,KACtB,CAEA,8BAAgB,CACZ,QAAQ,CAAE,QAAQ,CAClB,GAAG,CAAE,CAAC,CACN,IAAI,CAAE,CAAC,CACP,KAAK,CAAE,IAAI,CACX,MAAM,CAAE,IAAI,CACZ,OAAO,CAAE,CAAC,CACV,cAAc,CAAE,IAAI,CACpB,QAAQ,CAAE,OACd,CAEA,4BAAc,CACV,QAAQ,CAAE,QAAQ,CAClB,MAAM,CAAE,IAAI,CACZ,KAAK,CAAE,IAAI,CACX,KAAK,CAAE,IAAI,CACX,MAAM,CAAE,IAAI,CACZ,aAAa,CAAE,GAAG,CAClB,UAAU,CAAE,KAAK,CAAC,CAAC,CAAC,CAAC,CAAC,CAAC,CAAC,CAAC,CAAC,GAAG,CAAC,CAC9B,MAAM,CAAE,GAAG,CAAC,KAAK,CAAC,KAAK,GAAG,CAAC,CAAC,GAAG,CAAC,CAAC,GAAG,CAAC,CAAC,GAAG,CAAC,CAC1C,KAAK,CAAE,KAAK,GAAG,CAAC,CAAC,GAAG,CAAC,CAAC,GAAG,CAAC,CAAC,GAAG,CAAC,CAC/B,SAAS,CAAE,IAAI,CACf,MAAM,CAAE,OAAO,CACf,OAAO,CAAE,IAAI,CACb,WAAW,CAAE,MAAM,CACnB,eAAe,CAAE,MAAM,CACvB,OAAO,CAAE,EAAE,CACX,UAAU,CAAE,GAAG,CAAC,IAAI,CAAC,QACzB,CAEA,4BAAa,MAAO,CAChB,UAAU,CAAE,KAAK,CAAC,CAAC,CAAC,CAAC,CAAC,CAAC,CAAC,CAAC,CAAC,GAAG,CAAC,CAC9B,YAAY,CAAE,KAAK,GAAG,CAAC,CAAC,GAAG,CAAC,CAAC,GAAG,CAAC,CAAC,GAAG,CAAC,CACtC,KAAK,CAAE,KAAK,GAAG,CAAC,CAAC,GAAG,CAAC,CAAC,GAAG,CAAC,CAAC,CAAC,CAChC,CAEA,4BAAa,OAAQ,CACjB,SAAS,CAAE,MAAM,IAAI,CACzB,CAEQ,oBAAsB,CAC1B,KAAK,CAAE,IAAI,CACX,MAAM,CAAE,IACZ,CAEQ,sBAAwB,CAC5B,UAAU,CAAE,SAAS,CAAC,IAAI,CAAC,QAC/B,CAEQ,sBAAwB,CAC5B,cAAc,CAAE,IACpB"}'
};
const Graph = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let viewBox;
  let { nodes = [] } = $$props;
  let { links = [] } = $$props;
  let { width = window.innerWidth } = $$props;
  let { height = window.innerHeight } = $$props;
  ...
  do {
    ...
    viewBox = width ? `${width * mergedConfig.viewport.origin.x} ${height * mergedConfig.viewport.origin.y} ${width * mergedConfig.viewport.scale} ${height * mergedConfig.viewport.scale}` : "0 0 100 100";
    $$rendered = `  <div class="graph-container svelte-1vitirz"${add_attribute("this", container, 0)}><button class="reset-button svelte-1vitirz" aria-label="Reset view" data-svelte-h="svelte-tkzas9">⟲</button> <svg class="background-svg svelte-1vitirz"${add_attribute("width", width, 0)}${add_attribute("height", height, 0)}${add_attribute("viewBox", viewBox, 0)}${add_attribute("preserveAspectRatio", mergedConfig.viewport.preserveAspectRatio, 0)}${add_attribute("this", backgroundSvg, 0)}><g class="background-group"${add_attribute("this", backgroundGroup, 0)}></g></svg> ${validate_component(GraphLayout_1, "GraphLayout").$$render(
      $$result,
      {
        nodes,
        links,
      ...
      {
        node: ({ node, position }) => {
          return `${slots.node ? slots.node({ node, position }) : ``} `;
        },
        edge: ({ link, source, target }) => {
          return `${validate_component(Edge, "Edge").$$render($$result, { link, source, target }, {}, {})}`;
        }
      }
    )} </div>`;
  } while (!$$settled);
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    graphData.nodes;
    graphData.links;
    $$rendered = `  ${!isLoadingComplete ? `<div class="loading-container svelte-1nfodeq" data-svelte-h="svelte-uhd5sy"><div class="loading-spinner svelte-1nfodeq"></div> <span class="loading-text svelte-1nfodeq">Loading...</span></div>` : `${centralNode ? `${validate_component(Graph, "Graph").$$render(
      $$result,
      {
        nodes: graphData.nodes,
        links: graphData.links ?? [],
      ...
      {
        node: ({ node }) => {
          return `${isDashboardNode(node) ? `${validate_component(DashboardNode, "DashboardNode").$$render($$result, { node: node.data, userActivity }, {}, {})}` : `${isEditProfileNode(node) ? `${validate_component(EditProfileNode, "EditProfileNode").$$render($$result, { node: node.data }, {}, {})}` : `${isCreateNodeNode(node) ? `${validate_component(CreateNodeNode, "CreateNodeNode").$$render($$result, { node: node.data }, {}, {})}` : `${isWordNode(node) ? `${node.group === "central" ? `${wordNodeMode === "preview" ? `${validate_component(WordPreview, "WordPreview").$$render(
            $$result,
            {
              data: node.data,
              style: wordStyle,
