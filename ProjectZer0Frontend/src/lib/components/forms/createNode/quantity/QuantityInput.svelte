...
<script lang="ts">
    ...
    // For quantity questions, we want to require content
    $: isOverLimit = question.length > TEXT_LIMITS.MAX_STATEMENT_LENGTH;
    $: isEmpty = question.trim().length === 0;
    ...
    function handleInput(event: Event) {
        ...
        if (textarea.value.length > TEXT_LIMITS.MAX_STATEMENT_LENGTH) {
            question = textarea.value.slice(0, TEXT_LIMITS.MAX_STATEMENT_LENGTH);
        }
    }
...
<g>
    ...
    >
        <textarea
            ...
            placeholder="Enter a question about a quantifiable value (e.g., 'What is the average temperature in San Francisco?')"
            {disabled}
        />
    </foreignObject>
    ...
    <text 
        ...
        class:near-limit={question.length > TEXT_LIMITS.MAX_STATEMENT_LENGTH - 20}
        class:over-limit={isOverLimit}
        text-anchor="end"
    >
        {TEXT_LIMITS.MAX_STATEMENT_LENGTH - question.length} characters remaining
    </text>
