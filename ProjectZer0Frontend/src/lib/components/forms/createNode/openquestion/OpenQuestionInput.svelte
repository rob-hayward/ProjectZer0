...
<script lang="ts">
    ...
    // For questions, we want to require content and use the standard 280 char limit
    $: isOverLimit = questionText.length > TEXT_LIMITS.MAX_OPENQUESTION_LENGTH;
    $: isEmpty = questionText.trim().length === 0;
    ...
    function handleInput(event: Event) {
        ...
        if (textarea.value.length > TEXT_LIMITS.MAX_OPENQUESTION_LENGTH) {
            questionText = textarea.value.slice(0, TEXT_LIMITS.MAX_OPENQUESTION_LENGTH);
        }
    }
...
<g>
    ...
    >
        <textarea
            ...
            placeholder="What question would you like to pose to the ProjectZer0 community?"
            {disabled}
        />
    </foreignObject>
    ...
    <text 
        ...
        class:near-limit={questionText.length > TEXT_LIMITS.MAX_OPENQUESTION_LENGTH - 20}
        class:over-limit={isOverLimit}
        text-anchor="end"
    >
        {TEXT_LIMITS.MAX_OPENQUESTION_LENGTH - questionText.length} characters remaining
    </text>
