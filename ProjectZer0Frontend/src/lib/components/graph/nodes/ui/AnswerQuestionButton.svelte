<!-- src/lib/components/graph/nodes/ui/AnswerQuestionButton.svelte -->
<script lang="ts">
    ...
    const filterId = `answer-button-glow-${Math.random().toString(36).slice(2)}`;

    // Set button color to CYAN for OpenQuestion theme
    const buttonColor = "#00bcd4";

    // Button size matching DiscussButton
    ...
    function handleClick() {
        ...
        // Dispatch answerQuestion event with questionId
        dispatch('answerQuestion', { questionId });
        
        console.log(`[AnswerQuestionButton] Button clicked for question: ${questionId}`);
    }
</script>
...
>
    ...
    <!-- Material Symbol icon for Lightbulb -->
    <text 
...
<style>
    ...
    .answer-button:hover:not(.disabled) .button-circle {
        stroke-width: 2.5;
    }
