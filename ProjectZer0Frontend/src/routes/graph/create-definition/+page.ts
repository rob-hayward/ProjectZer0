// src/routes/graph/create-definition/+page.ts
...
export const load = (async ({ url }): Promise<GraphPageData> => {
    ...
    try {
        ...
        console.debug('[CREATE-DEFINITION-ROUTE] Word data loaded successfully', {
            wordId: wordData.id,
            word: wordData.word,
        ...
        return {
            ...
            viewType: 'create-definition',
            wordData,
            statementData: null,
            _routeKey: Math.random().toString(36).substring(2) // Random key for forcing re-renders
        };
    } catch (error) {
        console.error('[CREATE-DEFINITION-ROUTE] Error loading word data:', error);
        throw redirect(307, '/graph/dashboard');
    }
}) satisfies PageLoad;