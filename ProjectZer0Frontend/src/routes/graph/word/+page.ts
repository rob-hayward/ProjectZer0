...
export const load = (async ({ url }): Promise<GraphPageData> => {
    console.debug('[WORD-ROUTE] Loading word view');

    // Get word param from URL
    const wordParam = url.searchParams.get('word');
    if (!wordParam) {
        console.debug('[WORD-ROUTE] No word parameter, redirecting to dashboard');
    ...
    try {
        ...
        if (!wordData) {
            console.debug('[WORD-ROUTE] Word data not found, redirecting to dashboard');
            throw redirect(307, '/graph/dashboard');
        }
