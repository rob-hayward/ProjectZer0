<!-- src/routes/graph/edit-profile/+page.svelte -->
<script lang="ts">
    ...
    const viewType: ViewType = 'edit-profile';
    ...
    async function initializeData() {
        ...
        try {
            ...
            console.log('[EDIT-PROFILE] User authenticated', {
            ...
            console.error('[EDIT-PROFILE-ERROR] Error in initializeData:', error);
            auth0.login();
        }
    }
    ...
    function handleNodeModeChange(event: CustomEvent<{ nodeId: string; mode: NodeMode }>) {
        console.log('[EDIT-PROFILE] Node mode change', {
            nodeId: event.detail.nodeId,
            newMode: event.detail.mode
        });
    }

    // Create edit profile node
    ...
    // Get navigation options for edit profile context
    $: navigationNodes = getNavigationOptions(NavigationContext.EDIT_PROFILE)
        .map(option => ({
            id: option.id,
    ...
    function createGraphData(): GraphData {
        ...
        console.log('[EDIT-PROFILE] Created graph data', {
